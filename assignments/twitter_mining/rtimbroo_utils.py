...
##------------------------------------------------------------------------
def generateFeatureVector(input_ex, classLabel, logger):
    import re
    ...
    logger.info("Instance Name:[{0}], TargetLabel:[{1}] Total lines in inputExLines file:[{2}]".format(instanceName,classLabel,len(inputExLines)))
    ...
    logger.info("non-header lines count:[{0}]".format(len(lines)))
...
##################################################################################################################
def getFilesFromPath(path,FILE_SUFFIX, logger):
    import os
