...
'''
'''
def inst_vectorizer(ngram_type,vectorizer_type,binary=False,input='filename',max_df=1.0,min_df=1,analyzer='word',max_features=None):
    ...
    if vectorizer_type == 'count':
        vectorizer = CountVectorizer(input=input,binary=binary,ngram_range=ngram,max_df=max_df,min_df=min_df,analyzer=analyzer,max_features=max_features)
    elif vectorizer_type == 'tfidf':
        vectorizer = TfidfVectorizer(input=input,binary=binary,ngram_range=ngram,max_df=max_df,min_df=min_df,analyzer=analyzer,max_features=max_features)
    else:
        vectorizer = CountVectorizer(input=input,binary=binary,ngram_range=ngram,max_df=max_df,min_df=min_df,analyzer=analyzer,max_features=max_features)
