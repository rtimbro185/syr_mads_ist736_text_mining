...
'''
import logging
def train_vector(logger, vector, files):
    import pandas as pd
    ...
    logger.info(f'Vectorizer transformed shape: {transform.shape}')
    logger.info(f'Vectorizer transformed size: {transform.size}')
    logger.info(f'Vectorizer transformed type: {type(transform)}')
    logger.info(f'Vectorizer vocabulary size: {len(vector.vocabulary_)}')
    ...
    s = voc_df.sort_values(by='feature_index', ascending=False)[::10].head(20)
    logger.info(f'Vector Head:\n {s}')
