...
'''
'''
def inst_vectorizer(ngram_type,vectorizer_type,binary=False,input='filename',max_df=1.0,min_df=1,analyzer='word',max_features=None,stop_words=None):
    ...
    if vectorizer_type == 'count':
        vectorizer = CountVectorizer(input=input,binary=binary,ngram_range=ngram,max_df=max_df,min_df=min_df,analyzer=analyzer,max_features=max_features,stop_words=stop_words)
    elif vectorizer_type == 'tfidf':
        vectorizer = TfidfVectorizer(input=input,binary=binary,ngram_range=ngram,max_df=max_df,min_df=min_df,analyzer=analyzer,max_features=max_features,stop_words=stop_words)
    else:
        vectorizer = CountVectorizer(input=input,binary=binary,ngram_range=ngram,max_df=max_df,min_df=min_df,analyzer=analyzer,max_features=max_features,stop_words=stop_words)
...
'''
'''
def plot_bar_(df,x,y,title,ylab,xlab,sort_by,bar_label='Total'):
    import seaborn as sns
    import matplotlib.pyplot as plt
...
def cleanTagsAndFlatten(doc):
    logger.debug("Entering.cleanTagsAndFlatten; Cleaning Document [%s]"%doc)
    #if isTest: outfile = open('outputTagsClean.txt','a')
    docLines=[]
    ...
    #if isTest: outfile.write(' '.join(docLines)); outfile.write('\n')
    logger.debug("Exiting.cleanTagsAndFlatten; Returning: ")
...
def cleanNonWordChars(doc):
    logger.debug("Entering.cleanNonWordChars; Cleaning Document")
    ...
    logger.trace("Exiting.cleanNonWordChars; Returning:")
    return docWords

##################################################################################################################
# Function: Utility function to 
##################################################################################################################
def display_topics(model, feature_names, no_top_words):    
    for topic_idx, topic in enumerate(model.components_):
        logger.info("Topic %d:" % (topic_idx))
        logger.info(" ".join([feature_names[i] for i in topic.argsort()[:-no_top_words - 1:-1]]))
